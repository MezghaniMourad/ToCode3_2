		...
		if(!$error){
			...
			if ($err) {
			 ...
			 try {
					...
					$result1 = $client->call('countryDetails', array('CountryCode'=>$_POST['pays']));
...
if($res =='Country not found in the database'){
    ...
    echo '<h4 style="color:red;text-align:center;">Pays non trouve dans la base de donnees !!! Veuillez entrez un nom de pays valide </h4></br>'.$result1;

}
else {
echo " <p style=\"text-align: center;\"> La devise de la <strong>".$_POST['pays']."</strong> : ".($result['CountryCurrencyResult']['sISOCode'])."(".($result['CountryCurrencyResult']['sName']).")"."</p></br>".$result1;
